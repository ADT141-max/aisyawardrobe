...
export const ProductCardItem = ({ product, openDetail }) => {
  ...
  if (!product) return null; // Pelindung jika produk kosong
    ...
    <div onClick={() => openDetail(product)} className="bg-white rounded-2xl shadow-sm border border-stone-100 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col group relative cursor-pointer">
        ...
        {product?.images?.length > 1 ? (
          <div className="flex overflow-x-auto snap-x snap-mandatory h-full w-full no-scrollbar" onClick={e => e.stopPropagation()}>
            {product.images.map((img, idx) => (
              <img key={`pc-img-${idx}`} src={img} alt={`${String(product?.name || 'Item')}`} className="w-full h-full object-cover shrink-0 snap-center lg:group-hover:scale-110 transition-transform duration-700" />
            ))}
          </div>
        ) : (
          <img src={product?.images?.[0] || product?.image || 'https://placehold.co/400?text=No+Image'} alt={String(product?.name || 'Item')} className="w-full h-full object-cover lg:group-hover:scale-110 transition-transform duration-700" />
        )}
        
        <div className="absolute top-4 right-4 flex flex-col items-end gap-2">
           <div className="bg-white/90 backdrop-blur px-3 py-1.5 rounded-full text-[10px] font-bold text-stone-800 uppercase tracking-wider shadow-sm pointer-events-none">{String(product?.category || 'Umum')}</div>
      ...
      <div className="p-6 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-1">
           <h3 className="font-serif font-bold text-xl text-stone-900 leading-tight group-hover:text-amber-600 transition-colors pr-2">{String(product?.name || 'Tanpa Nama')}</h3>
        </div>
        
        <div className="flex gap-2 items-center mb-4">
          <p className="text-[10px] text-stone-400 font-mono bg-stone-50 px-2 py-1 rounded w-max">{String(product?.id || 'ID-XXX')}</p>
        </div>

        <div className="mb-4">
          {isPromo ? (
            <div className="flex flex-col">
               <span className="text-xs text-stone-400 line-through">Normal: {formatRupiah(product?.price || 0)}</span>
            ...
            <p className={`${cTheme?.text || 'text-rose-600'} font-bold text-xl`}>{formatRupiah(effPrice)} <span className="text-sm font-light text-stone-500">/ hari</span></p>
          )}
          {(product?.deposit || 0) > 0 && <p className="text-xs font-bold text-amber-600 bg-amber-50 inline-block px-2 py-1 rounded mt-2">+ Deposit: {formatRupiah(product.deposit)}</p>}
        </div>
        ...
        <div className="mt-auto pt-5 border-t border-stone-100 flex items-center justify-between">
          ...
          <button className={`px-5 py-2.5 rounded-full font-bold transition-all active:scale-95 flex items-center gap-2 ${avail > 0 ? `${cTheme?.bg || 'bg-rose-600'} text-white shadow-md hover:shadow-lg` : 'bg-stone-100 text-stone-400 cursor-not-allowed'}`}>Detail & Sewa</button>
        </div>
      </div>
    </div>
  );
...
export const DashboardView = () => {
  ...
  // Perhatikan perlindungan p && p?.status di sini!
           ...
           <div className="bg-white rounded-3xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row shadow-2xl relative animate-fade-in-down" onClick={e => e.stopPropagation()}>
              ...
              <div className="w-full md:w-1/2 p-6 md:p-10 flex flex-col overflow-y-auto no-scrollbar">
                  ...
                  <div className="mb-8 bg-stone-50 p-5 rounded-2xl border border-stone-100 relative">
                      ...
                      {hasDiscount ? (
                        <><p className="text-sm text-stone-400 line-through mb-1">Normal: {formatRupiah(selectedDetail?.price || 0)}</p><p className="text-red-600 font-bold text-3xl mb-2">{formatRupiah(selectedDetail?.discountPrice || 0)} <span className="text-sm font-light text-stone-500">/ hari</span></p></>
                      ) : (<p className={`${cTheme?.text || 'text-rose-600'} font-bold text-3xl mb-2`}>{formatRupiah(selectedDetail?.price || 0)} <span className="text-sm font-light text-stone-500">/ hari</span></p>)}
                      {(selectedDetail?.deposit || 0) > 0 && <p className="text-xs font-bold text-amber-600 bg-amber-100/50 inline-block px-3 py-1.5 rounded-lg border border-amber-200">+ Deposit Jaminan: {formatRupiah(selectedDetail.deposit)}</p>}
                  </div>
                  ...
                  <div className="mb-8"><h4 className="text-sm font-bold text-stone-800 tracking-wider mb-3 border-b border-stone-100 pb-2">Deskripsi Pakaian</h4><p className="text-base text-stone-600 leading-relaxed font-light whitespace-pre-line">{String(selectedDetail?.desc || '')}</p></div>
                  ...
                  <div className="mt-auto pt-6 border-t border-stone-100 space-y-4">
                      ...
                      <div className="flex gap-3">
                          ...
                          <button onClick={(e) => { e.stopPropagation(); addToCart(selectedDetail, selectedSize); closeDetail(); }} disabled={!canAddSelected} className={`flex-1 py-4 rounded-xl font-bold transition-all active:scale-95 flex items-center justify-center gap-2 text-lg shadow-lg ${canAddSelected ? `${cTheme?.bg || 'bg-rose-600'} text-white hover:brightness-110` : 'bg-stone-100 text-stone-400 cursor-not-allowed'}`}>
  ...
  return (
    <div className="space-y-12 md:space-y-16 animate-fade-in-down w-full flex-grow">
      <div className={`relative ${cTheme?.bg || 'bg-rose-900'} rounded-[2.5rem] overflow-hidden shadow-2xl min-h-[400px] flex items-center`}>
        ...
        <div className="relative z-10 px-8 md:px-16 py-12 max-w-3xl">
          ...
          <h1 style={{ fontFamily: db?.brandConfig?.logoFont || 'Playfair Display' }} className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight text-white drop-shadow-lg">{String(db?.brandConfig?.appName || 'Aisya Wardrobe')}</h1>
          <p className="text-lg md:text-xl text-stone-200 mb-10 font-light max-w-xl leading-relaxed">{String(db?.brandConfig?.slogan || 'Penyewaan Gaun Premium')}</p>
...
export const CatalogView = () => {
  ...
  // Pelindung Utama Katalog: Memastikan produk (p) benar-benar ada sebelum dibaca atributnya
    ...
    <>
      <div className="animate-fade-in-down w-full flex-grow">
        ...
        <div className="relative mb-6 md:mb-8 max-w-3xl mx-auto">
          ...
          <input 
            type="text" 
            className="w-full pl-14 pr-6 py-4 border border-stone-200 rounded-full bg-white focus:outline-none focus:ring-2 focus:ring-rose-200 text-[16px] shadow-sm transition-all" 
            placeholder="Cari nama atau ID pakaian..." 
            value={searchQuery} 
            onChange={(e) => { 
        ...
        <div className="flex overflow-x-auto pb-4 mb-8 gap-3 no-scrollbar justify-start md:justify-center">
          <button 
            ...
            className={`px-6 py-2.5 rounded-full font-bold transition-all whitespace-nowrap ${activeCategory === 'Semua' ? 'bg-stone-900 text-white shadow-md' : 'bg-white text-stone-600 border border-stone-200 hover:bg-stone-50'}`}
          >
            Semua Koleksi
          </button>
          ...
          {safeCategories.map((cat, idx) => (
            <button 
              ...
              className={`px-6 py-2.5 rounded-full font-bold transition-all whitespace-nowrap ${activeCategory === cat ? `${cTheme?.bg || 'bg-rose-600'} text-white shadow-md` : 'bg-white text-stone-600 border border-stone-200 hover:bg-stone-50'}`}
            >
              {String(cat)}
            </button>
          ))}
